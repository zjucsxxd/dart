...
void MyWorld::solve() {
    ...
    for (int i = 0; i < numIter; i++) {
        ...
        mSkel->setConfig(newPose); // DART updates all the transformations based on newPose
    }
}
...
VectorXd MyWorld::updateGradients() {
    ...
    // compute J(q)
    Vector4d offset;
    offset << mSkel->getMarker(mConstrainedMarker)->getLocalCoords(), 1; // Create a vector in homogeneous coordinates
    // w.r.t ankle
